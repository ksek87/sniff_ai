...
def scrape_fragrance_links(driver):
    """Scrape fragrance links from the main fragrance page."""
    links_scraped = []
...
def scrape_fragrance_data(lnk, driver):
    """Scrape fragrance data from individual fragrance pages."""
    driver.get(lnk)
    ...
    name = soup.find('h1',
                     class_='text-h1Mobile md:text-h1 font-secondary mt-6 mb-1.5').text.strip()
    ...
    concepts = [concept.strip().strip(',')
                for concept in concepts_dl.get_text(strip=True).split(',')]
...
def save_new_to_database(data):
    """Save new fragrance data to the database."""
